...
public class CrossbowMixin extends RangedWeaponItem implements Vanishable {
    @Unique
    ...
    @Unique
    private static int getUpgradedStorageLevel(ItemStack itemStack) {
        return EnchantmentHelper.getLevel(ModContent.STORAGEUP, itemStack);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        int currentStorageSize = (getUpgradedStorageLevel(itemStack) == 0 ? 6 : 10);
        if (user.isSneaking() && getProjectiles(itemStack).size() < currentStorageSize) setCharged(itemStack, false);
        if (isCharged(itemStack)) {
            shootAll(world, user, hand, itemStack, getSpeed(itemStack), 1.0F);
            if (getProjectiles(itemStack).isEmpty()) setCharged(itemStack, false);
            return TypedActionResult.consume(itemStack);
        } else if (!user.getProjectileType(itemStack).isEmpty() && getProjectiles(itemStack).size() < currentStorageSize) {
            if (!isCharged(itemStack)) {
                this.charged = false;
                this.loaded = false;
    ...
    private static boolean loadProjectiles(LivingEntity shooter, ItemStack crossbow) {
        ...
        if (!loadProjectile(shooter, crossbow, itemStack, false, bl)) {
            return false;
        }
    ...
    @Unique
    private static void shootAll(World world, LivingEntity entity, Hand hand, ItemStack stack, float speed, float divergence) {
        ...
        ItemStack itemStack = (ItemStack)list.get(id);
